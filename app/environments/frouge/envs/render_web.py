...
class RenderWeb:
    ...
    @ui.refreshable
    def _gui_board(self, env, callback):
        board = env.board
        if board != None:
                        ...
                        with ui.column().classes("gap-0"):
                            for r in range(3):
                                ...
                                if env.phase == 0 and cell == CS and rider == "-":
                                    ...
                                    if rider.endswith("r"):
                                        self._element_rouleur(_get_rider_color(board,i,r)).style(f'width: 3em; height: 3em; {style_border}; background-color: {bg_c}')
                                    elif rider.endswith("s"):
                                        self._element_sprinteur(_get_rider_color(board,i,r)).style(f'width: 3em; height: 3em; {style_border}; background-color: {bg_c}')
                                    else:
    ...
    @ui.refreshable
    def _gui_players(self, env: FlammeRougeEnv):
        with ui.row():
    ...
    @ui.refreshable
    def _gui_human_actions(self, env: FlammeRougeEnv, callback, suggested_action = None):
            ...
            with ui.column():
                ...
                with ui.row().bind_visibility_from(env,"phase",backward=lambda p: (p==1) and (env.pov_player == env.current_player)):
                    with ui.button("Choose Sprinter deck", on_click=lambda:_on_select_sprinteur_deck(env, callback)):
                        self._element_sprinteur(_get_player_color(env.current_player+1)).style("width: 100%;")
                    ...
                    with ui.button("Choose Rouleur deck", on_click=lambda:_on_select_rouleur_deck(env, callback)):
                        self._element_rouleur(_get_player_color(env.current_player+1)).style("width: 100%;")
    ...
    def init_web(self, env: FlammeRougeEnv, callback = None):
        with ui.row().classes("bg-green-1 q-py-md").style("width: 100%;"):
            self._gui_board(env, callback)
        with ui.row():
        ...
        self._gui_players(env)
        self._gui_human_actions(env, callback)


    def render_web(self, env: FlammeRougeEnv, callback, suggested_action = None, **kwargs):
        self._gui_board.refresh(env, callback)
        self._gui_human_actions.refresh(env, callback, suggested_action)
        self._gui_players.refresh(env)

    @ui.refreshable
    def _element_rouleur(self, color: str):
        content = f'''
    ...
    @ui.refreshable
    def _element_sprinteur(self, color: str):
        content = f'''
